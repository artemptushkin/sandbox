...
Здесь хочу попробовать привести пример близкий к промышленному, но в сокращенной версии, чтобы ответить на вопрос зачем и как все же писать чище.
...
Смотря на интерфейс, внимание сосредоточено на то, **как** могут быть использованы реализации в будущем, потому что нет лишнего
```java
...
что нам не нужно, так как данный класс должен принимать *только* объект оборачиваемого типа. В противном случае смысл обертки пропадает
```java
/* Оборачиваемый класс */
