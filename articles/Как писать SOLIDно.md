...
Для любого изменения, связанного с `CardService`:
  - `AccountService` в библиотеке: придется изменить `AccountService` и пересобрать библиотеку или писать адаптер в нашем репозитории.
  Если бы был выделен интерфейс CardService - можно было бы изменить реализацию, передав в конструктор.
  - `AccountService` в текущем проекте: придется изменить `AccountService` в текущем проекте, соответственно его тесты.
  Звучит несложно, но если вы рефакторите модуль, таких изменений будет больше.
Отмечу еще, что:
...
```
- С созданием интерфейса разработчик описывает основные свойства модуля и здесь следует стремиться абстрактно представить ожидания от будущих реализаций.
При этом, смотря на интерфейс, внимание сосредоточено на то, **как** могут быть использованы реализации в будущем, потому что нет лишнего
```java
...
и не приходят в голову такие вещи, как _a как тестировать private методы?_, потому что всегда **тестируется API**. Все private
методы же будут _протестированы_ автоматически, а мы проверяем лишь ожидания от интерфейса.
...
<br>&nbsp;&nbsp;&nbsp;&nbsp;PS. Пример про букву "L" до "S" выбран осознанно, так как я предлагаю думать не про аббревиатуры, а
